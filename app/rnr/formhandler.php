...
class FormHandler
{
    ...
    public static function Sender($FormId, $HtmlOut = true)
    {
        ...
        if ($HtmlOut) {
            return ('<input type="hidden" name="_rnrform" value="' . $key . '">');
        } else {
            return $key;
        }
    ...
    public function __construct()
    {
        ...
        if (isset($data['_rnrform'])) {
            ...
            if ($sender[1] == $_COOKIE['_f']) {
                $this->errors = $this->errors | RNRFORM_ERR_RELOAD;
            }

            if ($sender[2] != $_SERVER['REMOTE_ADDR']) {
                $this->errors = $this->errors | RNRFORM_ERR_IP;
            }

            if ($sender[3] != preg_replace('/http(s*):\/\//', '', $this->sender)) {
                $this->errors = $this->errors | RNRFORM_ERR_SERVER;
            }
