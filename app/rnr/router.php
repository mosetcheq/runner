    ...
    if (is_array($routes)) {
        ...
        if (count($request) > 0) {
            while (($route = array_shift($routes)) && (!$exit)) {
                ...
                if ($route['flags'] & RTR_REVERSE) {
                    $worker = array_reverse($worker);
                }

                if (!($route['flags'] & RTR_NOCHECKSIZE) && (count($worker) != count($route['route']))) {
                    $passed = false;
                }
                ...
                while (($element = array_shift($route['route'])) && ($passed)) {
                    ...
                    switch ($element[1]) {
                        ...
                        case RTR_REQUIRED:
                            ...
                            break;
                        case RTR_REGEXP:
                            if (preg_match("/{$element[0]}/", $u_item, $tmp) > 0) {
                                $params[] = $tmp[1];
                            } else {
                                $passed = false;
                            }
                ...
                if ($passed) {
                    ...
                    if (strpos($class, '*') !== false) {
                        $class = rtrim(str_replace('*', array_shift($params) ?? '', $class), '_\\');
                    }
                    ...
                    if ($route['flags']&RTR_API_MODE) {
                        $method = 'on' . ucfirst(strtolower($_SERVER['REQUEST_METHOD'])) . ucfirst($method);
                    }

                    if (($route['flags']&RTR_ADDPARAMS) && (count($worker) > 0)) {
                        $params = array_merge($params, $worker);
                    }

                    if (($route['flags']&RTR_ADDPARAMSARRAY) && (count($worker) > 0)) {
                        $params = array($worker);
                    }

                    $output = new RouterStatus($class, $method, RTR_OK, $params);
                    if (!($route['flags']&RTR_CONTINUE)) {
                        $exit = true;
                    }
			...
			if (!isset($output)) {
                $output = new RouterStatus($defaults->className, $defaults->method, RTR_NOTFOUND, $request);
            }

            return $output;
        } else {
            return new RouterStatus($defaults->className, $defaults->method, RTR_NONE);
        }
